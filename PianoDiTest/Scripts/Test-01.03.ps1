...
Invoke-LabTest -TestId 'Test-01.03' -OutputDir $OutputDir -ComputerName $ComputerName -FileServer $FileServer -TestCredential $TestCredential -BoardCredential $BoardCredential -DirectorCredential $DirectorCredential -UnauthorizedCredential $UnauthorizedCredential -AdminUser $AdminUser -TemporarySpn $TemporarySpn -SkipTgsRequest:$SkipTgsRequest -HoneyUser $HoneyUser -LookBackHours $LookBackHours -LookBackMinutes $LookBackMinutes -PurgeKerberosCacheAtEnd:$PurgeKerberosCacheAtEnd
